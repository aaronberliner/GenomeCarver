...
Autodesk.Cyborg.Note.body='Welcome to the GenomeCarver workspace. This workspace was created through a collaboration between The Bio/Nano/Programmable Matter research Group and the CaiLab based at the University of Edinburgh. 
We present GenomeCarver, a computational tool for the harvesting and packaging of biological parts from model genomes. GenomeCarver interfaces with various genomes, identifies regions of interest according to user specification and extraction extracts corresponding DNA sequences from the genome feature files (GFFs), checks the sequence’s compatibility with the selected standard, and finally outputs optimized primer sequences to amplify the parts from genomic DNA, adding necessary flanking sequences to standardize the parts.
Through its compatibility with multiple genomes and multiple parts standards, GenomeCarver bridges systems biology and synthetic biology, and greatly enriches synthetic biologists’ design toolbox. It complements many parts-based design tools which currently exist by supporting the Synthetic Biology Open Language (SBOL) standard. The BNPM group openly supports the SBOL community.';
